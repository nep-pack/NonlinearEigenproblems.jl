...
# run tiar
println("Run TIAR")
λ1,_,err_iar=tiar(Dnep;neigs=Inf,logger=0,maxit=100,tol=1e-5,check_error_every=1,errmeasure=err_measure)
# run Infinite Lanczos
println("Run the infinite Lanczos method")
λ,W=ilan(Dnep,neigs=Inf,logger=1,maxit=mm,tol=1e-6,check_error_every=Inf,errmeasure=err_measure)
