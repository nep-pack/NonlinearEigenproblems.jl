...
f1= S -> one(S); f2= S -> -S; f3= S -> S*sin(S); f4= S -> exp(-S)
...
λ1,_,err_iar=tiar(nep1;Neig=100,displaylevel=0,maxit=m,tol=1e-12,check_error_every=1,errmeasure=rel_err)
...
λ,W,_=ilan(nep;Neig=200,displaylevel=0,maxit=100,tol=1e-6,check_error_every=Inf,errmeasure=(λ,v)->rel_err(λ,v[n+1:end]))
