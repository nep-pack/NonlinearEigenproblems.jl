...
err_rel=(λ,v)->compute_resnorm(nep,λ,v)/((abs(λ)+abs(exp(nep.tauv[1]))*nA1+abs(exp(nep.tauv[2]))*nA2)*norm(v));

println("Run the infinite Lanczos method")
λ,W,V,H,ω,HH=ilan(Float64,nep;Neig=10,displaylevel=0,maxit=100,tol=eps()*100,check_error_every=Inf)

println("Number of computed eigenpairs: ", length(λ))
for j=1:length(λ)
    println("Residual of the eigepair ", j, "th = ",err_rel(λ[j],W[:,j]))
end
