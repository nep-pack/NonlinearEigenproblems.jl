...
nep=SPMF_NEP([[0 1 ; 1 1.0], [1 0 ; 0 0]], [s->one(s),s->exp(1im*s^2)]);

λ,V=contour_block_SS(nep,radius=1.0,neigs=6,N=10000,σ=0.1,K=2,L=1);

@show norm(compute_Mlincomb(nep,λ[1],V[:,1]))
@show norm(compute_Mlincomb(nep,λ[2],V[:,2]))


function contour_assu_old(
    ::Type{T},
    nep::NEP;
    tol::Real=sqrt(eps(real(T))), # Note tol is quite high for this method
    σ::Number=zero(complex(T)),
