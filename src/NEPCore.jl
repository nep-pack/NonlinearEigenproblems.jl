...
"""
    ...
    type LinEigSolver
        ...
        julia_eigs::Function  #eigs(A) from Julia (For sparse (non-Generalized)EP)
        julia_eig::Function   #eig(A) from Julia (For non-sparse (non-Generalized)EP)
        matlab_eigs::Function #eigs(A,B) from MATLAB (For sparse GEP)
        julia_eig2::Function  #eig(A,B) from Julia (For non-sparse GEP)

        solvers::Array{Function}

        function LinEigSolver()
            this = new()


            this.sorteigs = function(λ_t,d)
                return sortperm(abs(λ_t*ones(size(d,1))-d))
            end

            this.julia_eigs = function(A,B,λ_t,n)
                d,V = eigs(A,nev=n,sigma=λ_t)
                
                return d,V
            end

            this.julia_eig = function(A,B,λ_t,n)
                d,V = eig(A);

                I = this.sorteigs(λ_t,d);
                V = V[:,I];
                d = d[I];
            ...
            this.matlab_eigs = function(A,B,λ_t,n)
                ...
                @mput aa bb sigma nev
                @matlab begin
                    sigma = double(sigma);
                    nev = double(nev);
                    aa = double(aa);
                    bb = double(bb);
                    (V,d)=eigs(aa,bb,nev,sigma);
                end
            ...
            this.julia_eig2 = function(A,B,λ_t,n)
            ...
            this.solvers = [this.julia_eigs,this.julia_eig,this.matlab_eigs,this.julia_eig2];
            
            this.solve = function foo(A;B=eye(eltype(A),size(A)[1]),λ_t=0,nev=size(A)[1])
                #k = Number of eigenvalues required
                #λ_t = Target(Default = 0)
            
           
                #Based on the truth value of 2 statements: (isGeneralized?) and (!issparse?), we can have 4 different
                #values of solver_ind and solvers[solver_ind] will refer to the appropriate function call
                solver_ind = (B != eye(eltype(A),size(A)[1]))*2+(!this.issparse(A))+1;

                #Call the appropriate solver from solvers
                d,V = this.solvers[solver_ind](A,B,λ_t,nev);

                return d,V
            end

            #Check sparsity of A. NOTE::Sparsity of B is not checked(Maybe a problem?!)
            this.issparse = function(A)
                if(issparse(A))
                    return true
                else
                    return false
