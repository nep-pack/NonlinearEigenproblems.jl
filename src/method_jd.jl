...
function jd{T, T_orth<:IterativeSolvers.OrthogonalizationMethod}(::Type{T},
    ...
    for k=1:maxit
        ...
        λ,s = jd_inner_eig_solver(typeof(nep), T, proj_nep)
        s = s/norm(s)
...
function jd_inner_eig_solver(::Type{T_orig_nep}, T, proj_nep) where {T_orig_nep <: ProjectableNEP}
    λ,s = sgiter(T, proj_nep, 1)
    return λ, s
end

function jd_inner_eig_solver(::Type{T_orig_nep}, T, proj_nep) where {T_orig_nep <: PEP}
