...
#####################################################################################
println("\nRunning two-sided RFI(bordered) on pep0")
nep=nep_gallery("pep0");
At = [nep.A[1]',nep.A[2]',nep.A[3]'];
nept=PEP(At);
try
    λ,x =rfi_b(nep,nept,displaylevel=1);
catch e
    # Only catch NoConvergence
    isa(e, NoConvergenceException) || rethrow(e)
    println("No convergence because:"*e.msg)
...
nept=PEP(At);
try
    λ,x =rfi(nep,nept,displaylevel=1,λ=-50.0+0.0im);
catch e
    # Only catch NoConvergence
    isa(e, NoConvergenceException) || rethrow(e)
    println("No convergence because:"*e.msg)
...
#####################################################################################
println("\nRunning two-sided RFI(bordered) on dep_double")
nep=nep_gallery("dep_double");
At = [nep.A[1]',nep.A[2]'];
nept=DEP(At,nep.tauv);
try
    λ,x =rfi_b(nep,nept,displaylevel=1, λ=9.424777903529886im);
catch e
    # Only catch NoConvergence
    isa(e, NoConvergenceException) || rethrow(e)
    println("No convergence because:"*e.msg)
