...
λ,W,V=ilan(Float64,nep;Neig=100,displaylevel=0,maxit=100,tol=eps()*100,check_error_every=Inf,errmeasure=rel_err)
...
println("At the last iteration re-solve the projected problem with", `tiar`,"to see the convergence history")
λ,_,err=tiar(pnep;Neig=100,displaylevel=0,maxit=100,tol=eps()*100,check_error_every=1,errmeasure=err_lifted)
