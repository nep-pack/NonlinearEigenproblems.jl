...
@test (abs(Dc[c[1]]-λ) < 1e-10 || abs(Dc[c[2]]-λ) < 1e-10) #Complex conjugate eigenvalues



println("\n Testing in only Complex64 precision (32 bit in real and 32 bit in imaginary)")
λ,u =jd(Complex64, nep, tol=1e-4, maxit=80);
println("\n Resnorm of computed solution: ",compute_resnorm(nep,λ,u))
println("\n Smallest eigevalue found: \n λ: ",λ)
...
@test norm(compute_Mlincomb(nep,λ,u)) < 1e-4
@test (abs(Dc[c[1]]-λ) < 1e-4 || abs(Dc[c[2]]-λ) < 1e-4) #Complex conjugate eigenvalues



println("\n Testing a non-PEP")
nep2 = nep_gallery("real_quadratic")
λ,u =jd(Complex128, nep2, tol=1e-10, maxit=80);
