...
@testset "ILAN" begin
    ...
    @testset "DEP" begin
        ...
        _,_,V2,H2,ω2,HH2=ilan(nep,σ=0,γ=1;Neig=10,displaylevel=1,maxit=10,tol=eps()*100,check_error_every=1,v=v0)
    ...
    @testset "SPMF and DEP" begin
        ...
        nep=DEP([A1,A2,A3],[0,1,0.8])
        v0=rand(n)
        V,H,ω,HH=ilan_benchmark(nep,σ=0,γ=1;Neig=10,displaylevel=1,maxit=10,tol=eps()*100,check_error_every=Inf,v=v0)

        f1= S -> -S; f2= S -> one(S); f3= S -> exp(-S); f4= S -> exp(-0.8*S)
        nep=SPMF_NEP([one(A1),A1,A2,A3],[f1,f2,f3,f4])
        _,_,V2,H2,ω2,HH2=ilan(nep,σ=0,γ=1;Neig=10,displaylevel=1,maxit=10,tol=eps()*100,check_error_every=Inf,v=v0)
