...
IAR=@testset "IAR" begin
    @testset "accuracy eigenpairs" begin
        (λ,Q)=iar_chebyshev(dep,σ=3,Neig=5,v=ones(n),
    ...
    @testset "DGKS" begin
        (λ,Q,err,V)=iar_chebyshev(dep,orthmethod=DGKS,σ=3,Neig=5,v=ones(n),displaylevel=0,maxit=100,tol=eps()*100)
        @test norm(V'*V-eye(size(V,2)))<1e-6
     end

     @testset "User provided doubleGS" begin
         (λ,Q,err,V)=iar_chebyshev(dep,orthmethod=DoubleGS,σ=3,Neig=5,v=ones(n),displaylevel=0,maxit=100,tol=eps()*100)
         @test norm(V'*V-eye(size(V,2)))<1e-6
      end

      @testset "ModifiedGramSchmidt" begin
          (λ,Q,err,V)=iar_chebyshev(dep,orthmethod=ModifiedGramSchmidt,σ=3,Neig=5,v=ones(n),displaylevel=0,maxit=100,tol=eps()*100)
          @test norm(V'*V-eye(size(V,2)))<1e-6
      end

       @testset "ClassicalGramSchmidt" begin
           (λ,Q,err,V)=iar_chebyshev(dep,orthmethod=ClassicalGramSchmidt,σ=3,Neig=5,v=ones(n),displaylevel=0,maxit=100,tol=eps()*100)
           @test norm(V'*V-eye(size(V,2)))<1e-6
       end
    end
