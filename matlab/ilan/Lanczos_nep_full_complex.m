function [V,T,omega] = Lanczos_nep_full_complex(nep,k,q1)
...
for j=1:k
    ...
    %alpha=sum(sum(bsxfun(@times,conj(Z),Q)));
    alpha=Z(:)'*Q(:);
    if j>1
        %beta=sum(sum(bsxfun(@times,conj(Z),Qp)));
        beta=Z(:)'*Qp(:);
    end
    %gamma=sum(sum(bsxfun(@times,conj(Z),Qn)));
    gamma=Z(:)'*Qn(:);
    ...
    %T(j+1,j)=sqrt(sum(sum(bsxfun(@times,conj(Qn),Qn))));%=Z(:)'*W(:);
    %T(j+1,j)=sqrt(Qn(:)'*Qn(:));
    Qn=Qn/T(j+1,j);
